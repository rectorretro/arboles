...
struct Productos
{
    ...
    Productos *izq;
    Productos* dere;
}*inicio=NULL,*aux_p,*actual,*anterior;

void insertarNodo(Productos*&,int, string,string,string,string,int,double);
Productos *nodocreacion(int, string,string,string,string,int,double);

void ordenarNodos();
//void insertarNodo(int, string,string,string,string,int,double);
void mostrarNodo(Productos*,int);
void ordenar(int);
void especifico(int);
void borrar(int);
///funciones antiguas
...
if(actual != NULL){
  while (actual->siguiente != NULL)
    ...
    cout << "Dame el precio: ";
    cin >> Preciox;
    insertarNodo(actual,Clavex,NombreProductox,Distribuidorx,Tipox,Contenidox,cantidadx,Preciox);
}
...
Productos *nodocreacion(int Clavex,string NombreProductox,string Distribuidorx,string Tipox,string Contenidox,int cantidadx, double Preciox){
...
void insertarNodo(Productos *&productos,int Clavex,string NombreProductox,string Distribuidorx,string Tipox,string Contenidox,int cantidadx, double Preciox){
    if (productos == NULL){
            Productos *nuevo_nodo = nodocreacion(Clavex,NombreProductox,Distribuidorx,Tipox,Contenidox,cantidadx,Preciox);
            productos = nuevo_nodo;
    }else{
        int datoRaiz = productos->almacen.Clave;
        if(datoRaiz != Clavex){


            if(datoRaiz>Clavex){
            insertarNodo(productos->izq,Clavex,NombreProductox,Distribuidorx,Tipox,Contenidox,cantidadx,Preciox);
        }else{
            insertarNodo(productos->dere, Clavex,NombreProductox,Distribuidorx,Tipox,Contenidox,cantidadx,Preciox);
        }
...
void mostrarNodo(Productos* actual,int conter){
    if(actual == NULL){
        return;
    }else{
        mostrarNodo(actual->dere, conter+1);
        for (int i=0; i<conter; i++){
            cout << "---->";
        }
      cout <<  actual->almacen.Clave    << endl;
      cout <<   actual->NombreProducto << endl;
      cout <<  actual->Distribuidor   << endl;
      cout <<   actual->Contenido  << endl;
      cout <<  actual->Tipo    << endl;
      cout <<   actual->almacen.cantidad   << endl;
      cout <<      actual->almacen.Precio << endl;
        //
        mostrarNodo(actual->izq, conter+1);
    }
...
void recuperarDeArchivo()
{
    ...
    if(inicio == NULL){
        ...
        insertarNodo(inicio,Clavex, reemplazarSharp(NombreProductox), reemplazarSharp(Distribuidorx), reemplazarSharp(Tipox),reemplazarSharp(Contenidox), cantidadx,Preciox);
    }
    }
